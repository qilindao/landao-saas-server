...
class MenuRepo extends BaseRepository
{
    ...
    {
        $cacheKey = GlobalCacheKeyEnum::LAN_DAO_MENU;
...
//        })->orderBy('order_no', 'ASC')->get(self::MENU_COLUMNS)->toArray();
    ...
    {
        return Arr::first($this->getAllList(), function (array $menu) use ($menuId) {
    ...
    public function generatePermission(array $menuIdsList = []): array
    {
        ...
        foreach ($menuList as $item) {
            ...
            if (in_array($item['type'], [MenuTypeEnum::MENU_TYPE_CATALOG, MenuTypeEnum::MENU_TYPE_MENU])) {
                $menus[] = $item;
                continue;
            } elseif ($item['type'] == MenuTypeEnum::MENU_TYPE_PERMISSION) {//按钮权限
    ...
    public function addMenu(array $input): array
    {
        ...
        $this->transaction();
        try {
            $input['component'] = $input['type'] == MenuTypeEnum::MENU_TYPE_PERMISSION ? "" : $input['component'];
            $input['keep_alive'] = $input['type'] == MenuTypeEnum::MENU_TYPE_PERMISSION ? false : $input['keep_alive'];
